...
java -Xmx100g -jar juicer_tools_1.22.01.jar pre -r 10000 tmp.pairs ${name}.phase_allele${i}.hic /home/leeh7/iQTL/fasta/chr_size/${CC1}x${CC2}_chr_sizes.txt;
rm -rf test${i}.sorted.pairs test.pairs tmp.pairs; done


for((j=1;j<20;j++)); do java -Xmx20g -jar juicer_tools_1.22.01.jar dump observed NONE ${name}.phase_allele1.hic chr${j}_${CC1} chr${j}_${CC1} BP 10000 ${name}.phase_allele1_chr${j}.txt; done
for((j=1;j<20;j++)); do java -Xmx20g -jar juicer_tools_1.22.01.jar dump observed NONE ${name}.phase_allele2.hic chr${j}_${CC2} chr${j}_${CC2} BP 10000 ${name}.phase_allele2_chr${j}.txt; done
